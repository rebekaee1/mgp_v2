...
ШАГ 1 — на ПЕРВОЕ колебание/возражение ВСЕГДА только ОТРАБОТАЙ его, с эмпатией, БЕЗ оффера менеджера/мониторинга:
- «подумаю / посоветуюсь / попозже» → уточни, ЧТО именно обдумать (цена, отель или даты), и предложи помочь подобрать прямо сейчас.
- «дорого / дороговато» → уточни, что в приоритете (цена/отель/даты), и предложи подобрать выгоднее / в рамках бюджета.
- конкретный запрос («другой отель», «первая линия», «другие даты», «не нравится, хочу 5★ у моря») → ПРОСТО ПЕРЕПОДБЕРИ под пожелания.
⛔ На ШАГЕ 1 НЕ предлагай ни менеджера, ни мониторинг — сначала помоги. НИКОГДА не начинай ответ с «могу передать менеджеру».

ШАГ 2 — ТОЛЬКО если клиент колеблется ПОВТОРНО (снова «подумаю» / «дорого» / «просто подумаю» / прощается без конкретики) — мягко, отработав реакцию, дай ДВОЙНОЙ оффер (выбор за клиентом):
...
(Там, где раньше на повторном колебании / «разобрали много вариантов» ты предлагал ТОЛЬКО менеджера — теперь предлагай ОБА варианта.)

### Условия для ШАГА 2
Только когда: был показ туров (search_tours) И бюджет ≥ 200 000 ₽ И колебание НЕ первое И клиент ещё не оставил контакт / не готов бронировать.
...
# Детектор «стоп-сигналов» клиента (колебание/прощание/цена-возражение) для
# ДЕТЕРМИНИРОВАННОГО показа кнопки подписки (бридж рисует её при
# offer_subscription=true). СОЗНАТЕЛЬНО НЕ ловит:
#   • actionable-запросы («другой отель», «первая линия», «не нравится, хочу 5*») —
#     их надо ОТРАБАТЫВАТЬ (переподбор), а не предлагать мониторинг;
#   • вежливое «спасибо» само по себе.
# Оффер показываем по воронке (см. should_offer_subscription_button): только на
# ПОВТОРНОМ стоп-сигнале, не на первом.
_SUBSCRIPTION_OFFER_TRIGGER_RE = re.compile(
    r"(?:подума|посоветова|попозже|по позже|"
    r"дорог|дешевл|подешев|не по карман|не готов|не сейчас|подорож)",
    re.IGNORECASE,
)
...
class YandexGPTHandler:
    ...
    def should_offer_subscription_button(self, user_text: str) -> bool:
        ...
        try:
            ...
            if not _SUBSCRIPTION_OFFER_TRIGGER_RE.search(norm):
                return False
            # Воронка 2 шагов: оффер/кнопку показываем ТОЛЬКО на ПОВТОРНОМ
            # колебании (>=2-й стоп-сигнал). Первый — для отработки возражения
            # (уточнить/помочь), без оффера. Считаем стоп-сигналы в истории
            # клиента (full_history включает текущее сообщение).
            hesitations = 0
            for _m in (getattr(self, "full_history", None) or []):
                if not isinstance(_m, dict) or _m.get("role") != "user":
                    continue
                _c = (_m.get("content") or "").lower().replace("ё", "е")
                if _SUBSCRIPTION_OFFER_TRIGGER_RE.search(_c):
                    hesitations += 1
            return hesitations >= 2
        except Exception:
