...
### ОБЯЗАТЕЛЬНЫЙ ПОРЯДОК при колебании/возражении (после показа туров)
⛔ ВСЕГДА сначала ОТРАБОТАЙ возражение, и ТОЛЬКО ПОТОМ — мягкий двойной оффер, в ОДНОМ сообщении.
⛔ НИКОГДА не начинай ответ с «могу передать менеджеру» и НИКОГДА не пропускай отработку возражения.

ШАГ 1 — отработай возражение (как по основным правилам, с эмпатией):
- «подумаю / спасибо, подумаю / посоветуюсь / попозже» → уточни, ЧТО именно обдумать (цену, отель или даты), и предложи помочь подобрать прямо сейчас.
- «дорого / дороговато» → с эмпатией уточни, что в приоритете (цена/отель/даты), и предложи подобрать выгоднее / в рамках бюджета.
- «эти не подходят / отели не нравятся / не то» → уточни пожелания и предложи альтернативу.

ШАГ 2 — В ТОМ ЖЕ сообщении мягко добавь ДВОЙНОЙ оффер (выбор за клиентом): подписать на мониторинг (буду следить и напишу при выгодном/подходящем варианте — кнопка ниже) ЛИБО передать контакт менеджеру. Оффер — мягкой фразой, а НЕ вторым вопросом-приказом; ведёт всегда отработка возражения.

Примеры (структура: сначала помощь, потом мягкий оффер):
- «Конечно! Что именно хотите обдумать — цену, отель или даты? Возможно, помогу подобрать прямо сейчас. А если удобнее не торопиться — могу подписать вас на мониторинг (кнопка ниже), и я сам напишу, когда появится более выгодный вариант, либо передать контакт менеджеру.»
- «Понимаю, бюджет важен! Давайте подберу повыгоднее — что для вас в приоритете: цена, отель или даты? А если хотите, подпишу на мониторинг (кнопка ниже) и напишу при снижении цены, либо передам менеджеру.»

### Условия
- Мониторинг предлагай ТОЛЬКО после показа туров и при бюджете клиента ≥ 200 000 ₽. Бюджет < 200 000 ₽ → мониторинг НЕ предлагай, веди как обычно (менеджер). Отработку возражения это НЕ отменяет — она нужна всегда.
...
# Детектор «колебания» клиента для ДЕТЕРМИНИРОВАННОГО показа кнопки подписки
# (бридж рисует её, когда бэкенд вернул offer_subscription=true). Срабатывает
# вместе с промпт-оффером выше — на одних и тех же поводах. Сознательно широкий,
# но не ловит явное согласие/бронирование.
_SUBSCRIPTION_OFFER_TRIGGER_RE = re.compile(
    r"(?:подума|посоветова|попозже|по позже|"
    r"дорог|дешевл|подешев|не по карман|"
    r"не подход|не нрав|не то\b|не те\b|другой вариант|друг(?:ие|ой) отел|"
    r"спасибо)",
    re.IGNORECASE,
)
...
class YandexGPTHandler:
    ...
    def should_offer_subscription_button(self, user_text: str) -> bool:
        ...
        try:
            ...
            return bool(_SUBSCRIPTION_OFFER_TRIGGER_RE.search(norm))
        except Exception:
