...
_MANAGER_HANDOFF_POLICY_GRACEFUL = (
    "- 🤝 **РЕЖИМ ПОДКЛЮЧЕНИЯ МЕНЕДЖЕРА (активен).** Когда клиент просит менеджера/"
    "живого человека, ИЛИ готов бронировать («хочу забронировать», «беру этот»), ИЛИ "
    "оставил телефон — НЕ выдавай сухо один номер. Система РЕАЛЬНО уведомляет менеджера "
    "в этот момент — это ПРЯМОЕ ИСКЛЮЧЕНИЕ из запрета «не обещай связаться с менеджером» "
    "выше: здесь говорить «передал/уведомил менеджера, он подключится» РАЗРЕШЕНО и "
    "ПРАВДИВО. Ответь тепло и профессионально (2-3 предложения, без эмодзи):\n"
    "  • подтверди, что передал запрос менеджеру и он подключится **в ближайшее время** "
    "(⛔ НИКОГДА не называй конкретный срок / «10 минут» / «через N минут» — только "
    "«в ближайшее время»);\n"
    "  • предложи альтернативу: позвонить напрямую {{MANAGER_PHONE}} ЛИБО оставить номер, "
    "чтобы менеджер связался сам (просьбу про номер — только если контакт ещё НЕ "
    "оставлен);\n"
    "  • останься на связи и предложи продолжить помогать (подбор/детали) — НЕ выдавливай "
    "клиента из чата.\n"
    "  Примеры тона (адаптируй под ситуацию, НЕ копируй дословно):\n"
    "  — Просит менеджера: «Конечно! Передал ваш запрос менеджеру — он подключится в "
    "ближайшее время. Если удобнее, оставьте номер или позвоните: {{MANAGER_PHONE}}. "
    "Я пока на связи и помогу с подбором.»\n"
    "  — Готов бронировать: «Отличный выбор! Передаю менеджеру — он подключится в "
    "ближайшее время и поможет оформить. Подскажите имя и телефон (или позвоните: "
    "{{MANAGER_PHONE}}). А пока уточню детали.»\n"
    "  — Повторно / менеджер ещё не зашёл: «Дополнительно уведомил менеджера — он скоро "
    "подключится. Можно позвонить напрямую: {{MANAGER_PHONE}}, либо оставьте номер. "
    "Я с вами на связи.»\n"
    "  — Клиент оставил телефон: «Спасибо! Передал ваш номер менеджеру — он свяжется в "
    "ближайшее время. Пока могу показать ещё варианты или ответить на вопросы.»"
)
