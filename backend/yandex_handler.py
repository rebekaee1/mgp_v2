...
Ты можешь подписать клиента на мониторинг: мы сами напишем ему в этот же чат, когда по его запросу появится более подходящий или подешевеет тур. Клиенту ничего делать не нужно.

### ВОРОНКА из ДВУХ ШАГОВ при колебании/возражении (строго по порядку)

ШАГ 1 — на ЛЮБОЕ возражение/конкретику ВСЕГДА сначала ОТРАБОТАЙ его по §17.0, БЕЗ оффера менеджера/мониторинга:
- «дорого / дешевле / бюджет» → отработай возражение по §17.0 (присоединись, уточни комфортную сумму, защити ценность, умные альтернативы) — НЕ скатывайся в «дешевле» и НЕ предлагай мониторинг.
- «не нравится / не подходит / другой отель / первая линия / другие даты» → ПЕРЕПОДБЕРИ под пожелания.
...
- Клиент выбрал менеджера → обычные правила (имя + телефон, submit_client_request).
- Один двойной оффер за диалог; если клиент отказался от обоих — «напишите, когда будете готовы», не настаивай.
"""

# ── Таксономия сигналов для ДЕТЕРМИНИРОВАННОГО показа кнопки подписки ──
# (бридж рисует кнопку при offer_subscription=true). Основано на реальных
# репликах клиентов с прода + лучших практиках работы с возражениями.
#
# HANDLE — actionable-возражения/запросы: их НАДО ОТРАБАТЫВАТЬ (§17.0 / переподбор),
#   мониторинг НЕ предлагаем этим ходом (цена, качество, конкретика, рассрочка и т.п.).
_SUB_HANDLE_RE = re.compile(
    r"(?:дорог|дешевл|подешев|бюджет|не по карман|подорож|\bцен[аыуе]|"
    r"\bотел|номер|питани|звезд|звёзд|\bдат[аыуе]|перв.{0,4}лини|курорт|направлени|"
    r"рассрочк|кредит|обман|подвох|развод|опасн|страшн|\bвойн|"
    r"не нрав|не подход|\bне то\b|друг(?:ой|ие|ую)\s+(?:отел|вариант|курорт|дат))",
    re.IGNORECASE,
)
# LEAVE — клиент ЯВНО уходит/ставит на паузу → оффер уместен СРАЗУ.
_SUB_LEAVE_RE = re.compile(
    r"(?:вернус|попозже|по позже|\bпозже\b|\bпотом\b|не сейчас|"
    r"в друг(?:ой|ое) (?:раз|ден)|как.?нибудь потом|напишу позже|пока не готов)",
    re.IGNORECASE,
)
...
# Маркер уже сделанного оффера в диалоге (чтобы не повторять — «один оффер за диалог»).
_SUB_OFFER_MARK_RE = re.compile(
    r"(?:подписать вас на мониторинг|подписаться на мониторинг|"
...
# ⚠️ ПОЛНЫЙ список городов вылета Tourvisor (синхронизирован с
# get_dictionaries(type=departure), 77 городов + код 99 «Без перелёта»).
# Раньше здесь было лишь 15 городов, и guard ниже (строка ~2615) отклонял
# все остальные как «недоступные», хотя в Tourvisor они реально есть
# (баг: вылеты из Архангельска/Омска/Оренбурга и т.п. ложно резались).
_DEPARTURE_CITIES = {
    1: "Москва", 2: "Пермь", 3: "Екатеринбург", 4: "Уфа",
    5: "Санкт-Петербург", 6: "Челябинск", 7: "Самара",
    8: "Нижний Новгород", 9: "Новосибирск", 10: "Казань",
    11: "Краснодар", 12: "Красноярск", 13: "Сургут", 14: "Тюмень",
    15: "Кемерово", 16: "Новокузнецк", 17: "Калининград",
    18: "Ростов-на-Дону", 19: "Нижнекамск", 20: "Хабаровск",
    ...
    34: "Нижневартовск", 35: "Ханты-Мансийск", 36: "Благовещенск",
    37: "Якутск", 38: "Брянск", 39: "Минеральные Воды", 40: "Астрахань",
    41: "Сыктывкар", 42: "Улан-Удэ", 43: "Петропавловск-Камчатский",
    44: "Чита", 45: "Братск", 46: "Владикавказ", 47: "Курск",
    48: "Магнитогорск", 49: "Орск", 50: "Ульяновск", 51: "Чебоксары",
    52: "Томск", 53: "Абакан", 54: "Нальчик", 55: "Ставрополь",
    56: "Сочи", 61: "Набережные Челны", 62: "Симферополь", 63: "Анапа",
    64: "Ижевск", 65: "Пенза", 67: "Новый Уренгой", 85: "Ярославль",
    94: "Махачкала", 95: "Калуга", 96: "Грозный",
    99: "Без перелёта",  # Fix P3: departure=99 = туры без перелёта
    101: "Саранск", 102: "Череповец", 103: "Иваново", 104: "Киров",
    115: "Липецк", 116: "Геленджик", 117: "Петрозаводск", 118: "Псков",
    119: "Курган", 123: "Ноябрьск", 124: "Горно-Алтайск", 131: "Элиста",
}
...
_DEPARTURE_VALIDATION = [
    ...
    (r'(?:из|с)\s+сочи', 56),
    # ── Полный охват остальных городов вылета Tourvisor (анкер из/с + полная основа) ──
    (r'(?:из|с)\s+сургут\w*', 13),
    (r'(?:из|с)\s+тюмен\w*', 14),
    (r'(?:из|с)\s+кемеров\w*', 15),
    (r'(?:из|с)\s+новокузнецк\w*', 16),
    (r'(?:из|с)\s+калининград\w*', 17),
    (r'(?:из|с)\s+нижневартовск\w*', 34),
    (r'(?:из|с)\s+нижнекамск\w*', 19),
    (r'(?:из|с)\s+хабаровск\w*', 20),
    (r'(?:из|с)\s+омск\w*', 21),
    (r'(?:из|с)\s+иркутск\w*', 22),
    (r'(?:из|с)\s+владивосток\w*', 23),
    (r'(?:из|с)\s+(?:южно[\s-]*)?сахалинск\w*', 24),
    (r'(?:из|с)\s+барнаул\w*', 25),
    (r'(?:из|с)\s+воронеж\w*', 26),
    (r'(?:из|с)\s+волгоград\w*', 27),
    (r'(?:из|с)\s+оренбург\w*', 28),
    (r'(?:из|с)\s+архангельск\w*', 29),
    (r'(?:из|с)\s+мурманск\w*', 30),
    (r'(?:из|с)\s+саратов\w*', 31),
    (r'(?:из|с)\s+белгород\w*', 32),
    (r'(?:из|с)\s+(?:ханты[\s-]*мансийск\w*|мансийск\w*)', 35),
    (r'(?:из|с)\s+благовещенск\w*', 36),
    (r'(?:из|с)\s+якутск\w*', 37),
    (r'(?:из|с)\s+брянск\w*', 38),
    (r'(?:из|с)\s+(?:мин\w*[\s.]*вод\w*|минвод\w*)', 39),
    (r'(?:из|с)\s+астрахан\w*', 40),
    (r'(?:из|с)\s+сыктывкар\w*', 41),
    (r'(?:из|с)\s+улан[\s-]*удэ', 42),
    (r'(?:из|с)\s+(?:петропавловск\w*|камчат\w*)', 43),
    (r'(?:из|с)\s+чит[аыуео]\w*', 44),
    (r'(?:из|с)\s+братск\w*', 45),
    (r'(?:из|с)\s+владикавказ\w*', 46),
    (r'(?:из|с)\s+курск\w*', 47),
    (r'(?:из|с)\s+магнитогорск\w*', 48),
    (r'(?:из|с)\s+орск\w*', 49),
    (r'(?:из|с)\s+ульяновск\w*', 50),
    (r'(?:из|с)\s+чебоксар\w*', 51),
    (r'(?:из|с)\s+томск\w*', 52),
    (r'(?:из|с)\s+абакан\w*', 53),
    (r'(?:из|с)\s+нальчик\w*', 54),
    (r'(?:из|с)\s+ставропол\w*', 55),
    (r'(?:из|с)\s+(?:набережн\w*\s*челн\w*|наб[\.\s]*челн\w*)', 61),
    (r'(?:из|с)\s+симферопол\w*', 62),
    (r'(?:из|с)\s+анап\w*', 63),
    (r'(?:из|с)\s+ижевск\w*', 64),
    (r'(?:из|с)\s+пенз\w*', 65),
    (r'(?:из|с)\s+(?:нов\w*\s*уренго\w*|уренго\w*)', 67),
    (r'(?:из|с)\s+ярославл\w*', 85),
    (r'(?:из|с)\s+махачкал\w*', 94),
    (r'(?:из|с)\s+калуг\w*', 95),
    (r'(?:из|с)\s+грозн\w*', 96),
    (r'(?:из|с)\s+саранск\w*', 101),
    (r'(?:из|с)\s+черепов\w*', 102),
    (r'(?:из|с)\s+иванов\w*', 103),
    (r'(?:из|с)\s+киров\w*', 104),
    (r'(?:из|с)\s+липецк\w*', 115),
    (r'(?:из|с)\s+геленджик\w*', 116),
    (r'(?:из|с)\s+петрозаводск\w*', 117),
    (r'(?:из|с)\s+псков\w*', 118),
    (r'(?:из|с)\s+курган\w*', 119),
    (r'(?:из|с)\s+ноябрьск\w*', 123),
    (r'(?:из|с)\s+горно[\s-]*алтайск\w*', 124),
    (r'(?:из|с)\s+элист\w*', 131),
    # «Без перелёта» — всегда последним, чтобы не перебивать города
...
_DEPARTURE_VERIFY = {
    1: r'москв', 2: r'перм[иья]', 3: r'екатеринбург|еката|екб',
    ...
    12: r'красноярск', 13: r'сургут', 14: r'тюмен', 15: r'кемеров',
    16: r'новокузнецк', 17: r'калининград', 18: r'ростов',
    19: r'нижнекамск', 20: r'хабаровск', 21: r'омск', 22: r'иркутск',
    23: r'владивосток', 24: r'сахалинск', 25: r'барнаул', 26: r'воронеж',
    27: r'волгоград', 28: r'оренбург', 29: r'архангельск', 30: r'мурманск',
    31: r'саратов', 32: r'белгород', 34: r'нижневартовск',
    35: r'ханты|мансийск', 36: r'благовещенск', 37: r'якутск',
    38: r'брянск', 39: r'мин\w*[\s.]*вод|минвод', 40: r'астрахан',
    41: r'сыктывкар', 42: r'улан[\s-]*удэ', 43: r'петропавловск|камчат',
    44: r'чит[аыуео]', 45: r'братск', 46: r'владикавказ', 47: r'курск',
    48: r'магнитогорск', 49: r'орск', 50: r'ульяновск', 51: r'чебоксар',
    52: r'томск', 53: r'абакан', 54: r'нальчик', 55: r'ставропол',
    56: r'сочи', 61: r'набережн\w*\s*челн|наб[\.\s]*челн', 62: r'симферопол',
    63: r'анап', 64: r'ижевск', 65: r'пенз', 67: r'уренго',
    85: r'ярославл', 94: r'махачкал', 95: r'калуг', 96: r'грозн',
    99: r'без.*перел|поезд|автобус|ж[./]?д|трансфер',
    101: r'саранск', 102: r'черепов', 103: r'иванов', 104: r'киров',
    115: r'липецк', 116: r'геленджик', 117: r'петрозаводск', 118: r'псков',
    119: r'курган', 123: r'ноябрьск', 124: r'горно[\s-]*алтайск', 131: r'элист',
}
...
# включённым handoff (MAX/widget): система РЕАЛЬНО уведомляет менеджера, поэтому
# здесь разрешено вежливо сказать «передал менеджеру, подключится в ближайшее
# время» + предложить телефон/оставить номер и продолжить помогать.
_MANAGER_HANDOFF_POLICY_CURT = (
    "- ⛔ «Оператор» / «позовите менеджера» / «хочу менеджера» / «живой человек» / "
    "«соедините с человеком» → НЕМЕДЛЕННО дай телефон: «Свяжитесь с менеджером: "
    "{{MANAGER_PHONE}} — они помогут!» НЕ продолжай каскад, НЕ предлагай "
    "альтернативы, НЕ уговаривай остаться в чате. Один ответ — номер телефона."
)
_MANAGER_HANDOFF_POLICY_GRACEFUL = (
    "- 🤝 **РЕЖИМ ПЕРЕДАЧИ МЕНЕДЖЕРУ (активен).** Когда клиент просит менеджера/живого "
    "человека, ИЛИ готов бронировать («хочу забронировать», «беру этот»), ИЛИ оставил "
    "телефон — НЕ выдавай сухо один номер, но и НЕ обещай, что менеджер «подключится "
    "сейчас» (мы НЕ управляем его занятостью/графиком — ложное обещание оттолкнёт "
    "клиента). Система ПАРАЛЛЕЛЬНО уведомляет менеджера — это ИСКЛЮЧЕНИЕ из запрета «не "
    "обещай связаться с менеджером» выше: фраза «передал ваш запрос менеджеру» РАЗРЕШЕНА "
    "и ПРАВДИВА. ГЛАВНОЕ — забрать контакт. Ответь тепло и профессионально (2-3 "
    "предложения, без эмодзи):\n"
    "  • в ПЕРВУЮ очередь мягко попроси оставить номер телефона, чтобы менеджер связался "
    "(только если контакт ещё НЕ оставлен);\n"
    "  • скажи, что параллельно уже передал запрос менеджеру — ⛔ БЕЗ обещания времени и "
    "живого подключения: НИКОГДА не говори «менеджер сейчас подключится», «в течение N "
    "минут», «в ближайшее время», «ответит здесь» — мы не знаем, свободен ли он;\n"
    "  • можешь предложить позвонить напрямую {{MANAGER_PHONE}}, если клиенту удобнее или "
    "срочно;\n"
    "  • останься на связи и предложи продолжить помогать (подбор/детали).\n"
    "  Примеры тона (адаптируй под ситуацию, НЕ копируй дословно):\n"
    "  — Просит менеджера: «Конечно! Передал ваш запрос менеджеру. Чтобы он точно с вами "
    "связался, оставьте, пожалуйста, номер — или позвоните напрямую: {{MANAGER_PHONE}}. "
    "Я пока на связи и помогу с подбором.»\n"
    "  — Готов бронировать: «Отличный выбор! Чтобы оформить, оставьте, пожалуйста, имя и "
    "номер — менеджер свяжется и поможет с бронированием. Запрос я ему уже передал. "
    "А пока могу уточнить детали по этому варианту.»\n"
    "  — Клиент оставил телефон: «Спасибо! Передал ваш номер менеджеру — он свяжется с "
    "вами. Пока могу показать ещё варианты или ответить на вопросы.»\n"
    "  — Хочет консультацию менеджера по вариантам: «Хорошо, передал менеджеру, чтобы он "
    "проконсультировал. Оставьте, пожалуйста, номер — или позвоните: {{MANAGER_PHONE}}. "
    "А пока я и сам могу сравнить варианты — что важнее: цена, отель или даты?»"
)
...
class YandexGPTHandler:
    ...
    def should_offer_subscription_button(self, user_text: str) -> bool:
        """True → бридж покажет кнопку «🔔 Подписаться на мониторинг» в этом ответе.

        Детерминированный показ кнопки в момент колебания клиента, в паре с
        промпт-оффером (тот же повод), но независимо от решения LLM. Условия:
        тенант-пилот + был показ туров + бюджет ≥ порога + в последнем сообщении
        клиента есть «колебание/возражение» + клиент ещё не подписан в этом ходе.
        """
        try:
            if not self._subscription_enabled():
                return False
            ...
            if not budget or int(budget) < _SUBSCRIPTION_MIN_BUDGET:
                return False
            if getattr(self, "_pending_subscription", None):
                return False

            norm = (user_text or "").lower().replace("ё", "е")
            # actionable-возражение/запрос → отрабатываем, оффер не показываем
            if _SUB_HANDLE_RE.search(norm):
                return False

            hist = getattr(self, "full_history", None) or []
            # «один оффер за диалог»: если мониторинг уже предлагали в прошлых
            # ответах ассистента — не повторяем (защита от спама оффером).
            for _m in hist:
                if isinstance(_m, dict) and _m.get("role") == "assistant" \
                        and _SUB_OFFER_MARK_RE.search(_m.get("content") or ""):
                    return False

            # Явный уход или отказ → оффер уместен сразу (1 раз).
    ...
    def _load_system_prompt(self) -> str:
        ...
        # Manager-handoff policy: грациозный режим только для ассистентов с
        # включённым handoff (MAX или widget); остальным — прежнее «дай телефон».
        # Канал на момент сборки промпта ещё неизвестен (ставится позже), поэтому
        # гейтим по assistant_id на обоих каналах. Подставляем ДО {{MANAGER_PHONE}},
        # чтобы плейсхолдер телефона внутри политики тоже резолвился.
        _handoff_on = False
        try:
            import manager_handoff as _MH
            _aid = str(getattr(self.runtime_config, "assistant_id", "") or "")
            if _aid:
                _handoff_on = (
                    _MH.handoff_enabled(_aid, "max") or _MH.handoff_enabled(_aid, "widget")
                )
        except Exception:
            _handoff_on = False
        prompt = prompt.replace(
            "{{MANAGER_HANDOFF_POLICY}}",
            _MANAGER_HANDOFF_POLICY_GRACEFUL if _handoff_on else _MANAGER_HANDOFF_POLICY_CURT,
        )
    ...
    async def _dispatch_function(self, name: str, args: Dict) -> Any:
        ...
        if name == "get_current_date":
            ...
            # _DEPARTURE_CITIES = полный живой список городов вылета Tourvisor
            # (синхронизирован с get_dictionaries(type=departure)). Сюда попадают
            # только реально несуществующие коды (галлюцинация модели).
            if dep_code and dep_code not in _DEPARTURE_CITIES:
                ...
                return json.dumps({
                    "error": (
                        f"⛔ Город вылета с кодом {dep_code} не найден в справочнике Tourvisor. "
                        f"Сверься с актуальным списком через get_dictionaries(type=departure): "
                        f"если клиент назвал реальный город — возьми его ID оттуда и повтори поиск. "
                        f"Если города действительно нет — сообщи клиенту, что вылет недоступен, "
                        f"и предложи 2-3 ближайших доступных города (например, Москва, Санкт-Петербург)."
                    )
                }, ensure_ascii=False)
