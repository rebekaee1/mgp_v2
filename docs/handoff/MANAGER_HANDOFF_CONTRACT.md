...
```
**Ключевой инвариант v2:** «уведомить менеджера» ≠ «поставить ИИ на паузу».
...
- Жёсткие триггеры (`book_click`/`phrase`/`contact`/`manual`): `manager_alert` +
  **клиентское заверение** «менеджер уведомлён» (без эмодзи; просьба телефона,
  если контакта ещё нет).
- Мягкий (`booking_intent`): ТОЛЬКО `manager_alert` (тихо), без текста клиенту.
- `operator_mode=True` (пауза ИИ) ставится ИСКЛЮЧИТЕЛЬНО при РЕАЛЬНОМ заходе
  ...
  один раз шлём «Менеджер уже в чате и ответит вам здесь».
