...
**Машина состояний (MGP-owned) — модель v2 (2026-06-10):**
```
none ──(любой триггер)─► requested ──(/handoff take | ответ оператора)─► operator
  ▲                          │  (operator_mode=FALSE, ИИ ПРОДОЛЖАЕТ)        │
  └──── returned ◄───────────┴───(/handoff return | авто-возврат 10 мин)────┘
                                   (operator_mode=TRUE только здесь)
```
**Ключевой инвариант v2.1 (2026-06-10):** «уведомить менеджера» ≠ «поставить ИИ
на паузу», и клиенту на триггере НИЧЕГО не пишем (тихий handoff).
- НИ ОДИН триггер не ставит `operator_mode=True`. В `requested` ИИ **продолжает**
  вести диалог, фиксирует лид (телефон → `submit_client_request`).
- ВСЕ триггеры (`book_click`/`phrase`/`contact`/`booking_intent`): ТОЛЬКО
  `manager_alert` в ЛК (тихо). **Клиенту на триггере ничего не отправляем** —
  никакого «передал менеджеру»: это пугает и ломает естественный диалог.
  Менеджер видит алерт в ЛК и при желании заходит в чат.
- `operator_mode=True` (пауза ИИ) ставится ИСКЛЮЧИТЕЛЬНО при РЕАЛЬНОМ заходе
  менеджера: `/operator/handoff take` или `/operator/message`. На `take` клиенту
  ОДИН раз шлём «Менеджер уже в чате и ответит вам здесь» (момент паузы ИИ).
- Захват телефона/лида — штатным поведением ИИ при бронировании (без отдельной
  системной реплики). Тексты `ACK_*`/`request_ack_text` оставлены в коде как
  dormant (на случай возврата к «заверению»), но в v2.1 НЕ используются.
- **«Поздняя отмена»:** если менеджер перехватил (`operator_mode=True`), пока ИИ
  генерировал ответ, ответ ИИ не отправляется и не персистится (входящее уходит
  как `operator_inbound`, in-memory handler эвиктится) — клиент видит только
  сообщение менеджера.
- Авто-возврат (10 мин) применяется только к состоянию `operator`.
